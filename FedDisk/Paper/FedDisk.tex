...
\R{par:problemStatement}\Copy{par:problemStatement}{
...
\R{par:howEffectiveness}\Copy{par:howEffectiveness}{
...
\R{sec:discussion}\Copy{sec:discussion}{
...
%Another observation is that the more difficult the classification task is, the more significant impact can be observed. For example, the FEMNIST dataset classification task is the most difficult for a model to perform well as the dataset has 62 classes, many fewer than this number for MNIST (10 classes) and Chest-Xray (2 classes). Therefore, the proposed method has a more obvious impact on the FEMNIST dataset than on other datasets. Furthermore, the impact on the MNIST dataset is more apparent than on the Chest-Xray dataset.
